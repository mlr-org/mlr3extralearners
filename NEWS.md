...
* `classif.tabpfn` and `regr.tabpfn`: added support for the feature types `character`, `factor`, and `ordered`, which are passed to `tabpfn` as categorical columns and ordinally encoded there. Previously the features were converted to a numeric matrix, which ruled out categorical features even though `tabpfn` supports them.
