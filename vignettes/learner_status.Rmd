...
make_badge = function(pkg) {
  ...
  sprintf("[![CRAN Status](https://www.r-pkg.org/badges/version-ago/%s)](https://cran.r-project.org/package=%s)", pkg, pkg)
}
...
tbl[["id"]] = mlr3misc::pmap(list(tbl[["id"]], tbl[["mlr3"]]), function(id, pkg) {
  ...
  unlist()
colnames(tbl) = c("Key", "mlr3", "Upstream Packages", "Upstream Status", "Author")
format_table(
  tbl,
  list(
