...
To fill some out this meta-information, one has to go through the manual pages of the upstream packages.
...
Essentially all `mlr3` learners call some upstream function (like `rpart::rpart()` in the example above), and this function has default values.
...
It is recommended to only change the defaults for good reasons and document this properly when doing so.
...
It has the argument `custom_check`, which allows to specify custom constraints.
...
* The tests must be pass in the CI.
* Update the `NEWS.md` file and describe what you added the learner.
