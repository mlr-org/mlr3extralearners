...
Because the learner has the property `"weights"`, we insert the weights of the task if there are any, by calling `$.get_weights()` to retrieve them.
...
For weighted performance evaluation, provide task weights with role `weights_measure` and then score as usual.
If you want the same weights for both fitting and evaluation, assign the same column to both roles.

If it runs without erroring, that's a very good start!

### Weighted Training and Evaluation

The following example uses separate weight columns to show how the task columns with the role `weights_learner` influences model fitting and the role `weights_measure` influences performance scoring.

```{r, eval = rpart_available}
task = tsk("mtcars")
# define learner weights based on dedicated columns
task_wt = tsk("mtcars")$cbind(data.table::data.table(
  w_fit = ifelse(task$data(cols = "cyl")[[1L]] == 8, 3, 1),
  w_eval = ifelse(task$data(cols = "am")[[1L]] == 1, 2, 1)
))
# set column roles so the learner can recognize which columns determine the respective weights
task_wt$set_col_roles(cols = "w_fit", roles = "weights_learner")
task_wt$set_col_roles(cols = "w_eval", roles = "weights_measure")

# train, predict and evaluate on unweighted and weighted task 
learner_unweighted = LearnerRegrRpartSimple$new()
pred_unweighted = learner_unweighted$train(task)$predict(task)
mse_unweighted = pred_unweighted$score(msr("regr.mse"))

learner_weighted = LearnerRegrRpartSimple$new()
pred_weighted = learner_weighted$train(task_wt)$predict(task_wt)
mse_weighted = pred_weighted$score(msr("regr.mse"))

# mse of an unweighted learner to one with weights during training and evaluation
c(mse_unweighted = mse_unweighted, mse_weighted = mse_weighted)
```

### Autotest

To ensure that a learner correctly handles different task properties, feature types, and edge cases, `mlr3extralearners` relies on a standardized autotest framework.
The autotest systematically evaluates learners across many automatically generated tasks.
...
Below is an example for a simple regression learner:
...
Always add an explicit skip at the top of test files for all learners, for example:
```{r, eval = FALSE}
skip_if_not_installed("RpkgName")
```

This ensures tests fail gracefully on GitHub Actions, where we have a specific workflow that doesn't install packages listed in `Suggests`.
...
`Python`-based learners (via reticulate) **must be isolated from the main R session** during testing.
This is required for both correctness and stability.

In particular:

- Python learners often depend on Python `torch`, while some R learners depend on the R `torch` package, which loads `libtorch`.
- Loading multiple `libtorch` binaries (from R and Python, or from different builds) in the same R session can lead to crashes.
- Different Python learners often require conflicting package dependencies.

Therefore, all Python learner tests must be executed in a fresh R session using `callr`.
...
When documenting a learner, examples are crucial to help users understand how to use it.
The `mlr3extralearners` package uses roxygen2 templates to standardize examples across learners.

### Example Templates

There are three main approaches for learner examples:

1. **Standard template (`@template example`)**: Use this for most learners. It automatically generates a complete example that creates the learner, trains it on an appropriate task (sonar for classification, mtcars for regression, grace for survival), makes predictions, and scores them. If the learner has the `"importance"` property, it also prints the importance scores.

2. **Simple template (`@template simple_example`)**: Use this when the learner cannot easily run on standard tasks without additional configuration, or when training is very slow. This template only creates and prints the learner without training.

3. **Custom templates (`@template example_<name>`)**: Use these when a learner requires specific parameters or task setup that differs from the standard template.

### When to Use Custom Templates

Create a custom template in `man-roxygen/example_<name>.R` when:

* **The learner requires specific parameters to compute importance**: Some learners (e.g., `randomForest`, `rfsrc`, `ranger`) have the `"importance"` property but require an explicit parameter to be set (e.g., `importance = "TRUE"` or `importance = "permutation"`).

* **The learner requires a formula parameter**: Learners like `lme4` (mixed-effects models), `mgcv` (GAMs), or `flexsurv` need a custom formula to demonstrate their functionality.

* **The learner requires custom parameters for small datasets**: Some learners like `laGP` need parameter adjustments (e.g., `end = 15`) to work on small datasets like mtcars.

* **The learner requires a custom task or data setup**: Learners like `prioritylasso` or `blockForest` require a `blocks` parameter that partitions features.

* **The learner has special methods to demonstrate**: The `cmprsk.rfsrc` learner has cause-specific importance that should be shown in the example.
...
3. Reference the `id` variable (set via `@templateVar id <learner-id>` in the learner file)
...
In your learner's R file, add the template reference in the roxygen documentation:
...
For custom templates, replace `@template example` with `@template example_<name>`.
...
* Adding the line `.extralrns_dict$add("<learner-id>", <LearnerClassName>)` to the bottom of the learner file to add the learner to the learners `Dictionary`.
