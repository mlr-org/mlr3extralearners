...
Sometimes you might want to add parameters that are not present in the upstream function.
One example is the `mtry.ratio` parameter of `lrn("surv.ranger")`.
Such parameters should usually not have `default` values (because we are here not calling an upstream function with a default behavior) but instead have their value initialized during construction and have the `"required"` tag.
...
The following example assigns the `weights_learner`-role to a task column to show how weights influence model fitting.

```{r, eval = rpart_available}
task = tsk("mtcars")

# row ids with am == 1
row_ids_am_1 = task$row_ids[task$data(cols = "am") == 1L]

# unweighted learner
learner = LearnerRegrRpartSimple$new()
pred = learner$train(task)$predict(task)
pred$filter(row_ids_am_1)$score(msr("regr.mse"))

# define learner weights based on am-columns with value "1"
task_wt = tsk("mtcars")$cbind(data.table::data.table(
  w_fit = ifelse(task$data(cols = "am")[[1L]] == 1, 30, 1)
))
...
# weighted learner
learner_wt = LearnerRegrRpartSimple$new()
pred_wt = learner_wt$train(task_wt)$predict(task_wt)

pred_wt$filter(row_ids_am_1)$score(msr("regr.mse"))
```

Based on the Mean Squared Error for the subset of cars having `am == 1` it becomes evident, that the weighted learner focuses more on minimizing the error for those observations than the unweighted learner.
