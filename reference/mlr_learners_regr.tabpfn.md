...
In order to save a `LearnerRegrTabPFN` for later usage, it is necessary
...
a marshaled `LearnerRegrTabPFN` into R again, you then need to call
...
- `output_type` corresponds to the same argument of the `.predict()`
  method of the `TabPFNRegressor` class, but only supports the options
...
- [`LearnerRegrTabPFN$unmarshal()`](#method-LearnerRegrTabPFN-unmarshal)

- [`LearnerRegrTabPFN$clone()`](#method-LearnerRegrTabPFN-clone)
