...
test_that("autotest", {
  ...
  # expect_learner(learner, check_man = FALSE) # will not work because base class has new tags
...
test_that("eval protocol", {
  ...
  task = tsk("sonar")
  learner$train(task)
  expect_true(is.list(learner$model$meta$eval_protocol))
})
...
test_that("validation and inner tuning works", {
  ...
  learner$train(task)
  expect_named(learner$model$meta$eval_protocol, c("epoch", "train_loss", "valid_loss", "accuracy"))
  ...
  learner$train(task)
  expect_equal(learner$internal_valid_scores$accuracy, learner$model$meta$eval_protocol$accuracy[learner$internal_tuned_values$n_epoch])
  ...
  learner = lrn("classif.python.fastai", validate = 0.3, n_epoch = 10)
  learner$train(task)
  expect_equal(learner$internal_valid_scores$accuracy, learner$model$meta$eval_protocol$accuracy[10L])
  expect_null(learner$internal_tuned_values)
})
...
test_that("custom inner validation measure", {
  ...
  expect_named(learner$model$meta$eval_protocol, c("epoch", "train_loss", "valid_loss", "error_rate"))
  expect_list(learner$internal_valid_scores, types = "numeric")
  ...
  expect_named(learner$model$meta$eval_protocol, c("epoch", "train_loss", "valid_loss", "classif.ce"))
  expect_numeric(learner$model$meta$eval_protocol$classif.ce, len = 10)
  expect_list(learner$internal_valid_scores, types = "numeric")
  ...
  expect_named(learner$model$meta$eval_protocol, c("epoch", "train_loss", "valid_loss", "classif.logloss"))
  expect_numeric(learner$model$meta$eval_protocol$classif.logloss, len = 10)
  expect_list(learner$internal_valid_scores, types = "numeric")
  ...
  expect_named(learner$model$meta$eval_protocol, c("epoch", "train_loss", "valid_loss", "classif.auc"))
  expect_numeric(learner$model$meta$eval_protocol$classif.auc, len = 10)
  expect_list(learner$internal_valid_scores, types = "numeric")
...
test_that("marshaling works for classif.python.fastai", {
  learner = lrn("classif.python.fastai")
  task = tsk("iris")
