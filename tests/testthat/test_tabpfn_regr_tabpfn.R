...
test_that("other tests", {
  expect_true(callr::r(function() {
    ...
    learner = mlr3::lrn("regr.tabpfn", categorical_features_indices = 5)
    expect_error(learner$train(task))
    learner = mlr3::lrn("regr.tabpfn", categorical_features_indices = 1:3)
    learner$train(task)
    ...
    learner = mlr3::lrn("regr.tabpfn", device = "cpu")
    learner$train(task)
    ...
    learner = mlr3::lrn("regr.tabpfn", device = "auto")
    learner$train(task)
    ...
    lapply(dtypes, function(dtype) {
      learner = mlr3::lrn("regr.tabpfn", inference_precision = paste0("torch.", dtype))
