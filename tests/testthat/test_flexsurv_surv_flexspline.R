...
test_that("formula interface works", {
  expect_error(lrn("surv.flexspline", formula = "not_a_formula"))
  expect_error(lrn("surv.flexspline", formula = list("not_a_formula")))
  task = tsk("lung")

  form = survival::Surv(time, status) ~ age + sex
  learner = lrn("surv.flexspline", k = 1, formula = form)
  learner$train(task)
  ...
  lp = predict(model, newdata = task$data(), type = "lp")[[2L]]
  expect_equal(p$lp, lp)

  # use of `anc` changes coefficients
  learner2 = lrn("surv.flexspline", k = 1, formula = form, anc = list(gamma1 = ~ sex))
  learner2$train(task)
