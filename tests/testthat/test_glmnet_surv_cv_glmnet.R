...
test_that("stratified Cox model works", {
  ...
  expect_error(lrn("surv.cv_glmnet", strata = 42L))
  expect_error(lrn("surv.cv_glmnet", strata = "non_existing_column")$train(task))
  ...
  task$rbind(
    data = data.table(
      ...
      estrg_recp = 20,
      grade = 2L,
      hormone = 4L, # hormone is only 1 or 2 in training set
      menopause = 1L,
      nodes = 2L,
      prog_recp = 30,
      size = 25
