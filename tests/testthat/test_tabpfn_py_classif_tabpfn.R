...
test_that("autotest", {
  learner = lrn("classif.python.tabpfn")
...
test_that("marshaling works for classif.python.tabpfn", {
  learner = lrn("classif.python.tabpfn")
  task = tsk("iris")
...
test_that("categorical feature columns are encoded correctly", {
  ...
  learner = lrn("classif.python.tabpfn", categorical_features_indices = 5)
  expect_error(learner$train(task))
  learner = lrn("classif.python.tabpfn", categorical_features_indices = 1:3)
  learner$train(task)
...
test_that("device selection works", {
  ...
  learner = lrn("classif.python.tabpfn", device = "cpu")
  ...
  learner = lrn("classif.python.tabpfn", device = "auto")
  learner$train(tsk("iris"))
...
test_that("inference_precision works", {
  ...
  lapply(dtypes, function(dtype) {
    learner = lrn("classif.python.tabpfn", inference_precision = paste0("torch.", dtype))
...
test_that("random_state works", {
  ...
  learner1 = lrn("classif.python.tabpfn", predict_type = "prob")
  learner1$param_set$set_values(random_state = "None")
  learner1$train(task)
  learner2 = lrn("classif.python.tabpfn", predict_type = "prob")
