...
eXtreme Gradient Boosting regression using an **Accelerated Failure
Time** objective. Calls
...
`objective` set to `survival:aft` and `eval_metric` to `aft-nloglik`.
...
1.  `response`: the estimated survival time \\T\\ for each test
    observation.

2.  `lp`: a vector of linear predictors (relative risk scores), one per
    observation, estimated as \\-log(T)\\. Higher survival time denotes
    lower risk.
...
- [`LearnerSurvXgboostAFT$clone()`](#method-LearnerSurvXgboostAFT-clone)
